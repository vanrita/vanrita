...
class Hessian2Parser{
	 ...
	 * @return Ambigous <>
	 */
	function readUnpack($num = 1, $format = 'c') {
	    $c = unpack($format, $this->read($num));
	    return $c[1];
	}
	...
	function parseInt($code, $num){
	    $data = ($this->readUnpack() << 24) + ($this->readNum() << 16) 
	          + ($this->readNum() << 8) + $this->readNum();
	    return $data;
	}
	...
	function double4($code, $num){
		$num = ($this->readUnpack() << 24) + ($this->readNum() << 16) 
	          + ($this->readNum() << 8) + $this->readNum();
		return 0.001 * $num;
	...
	function string1($code, $num){
	    $chr = $this->read();
		$len = (($num - 0x30) << 8) + ord($chr);
		$str = $this->readUTF8Bytes($len);
		return $str;
	}
	...
	function readUTF8Bytes($len){
		...
		if (HessianUtils::isInternalUTF8() || mb_detect_encoding($string, 'UTF-8', true)) {
		    return $string;
		}
